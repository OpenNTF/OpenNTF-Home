	...
	<fieldset>
		...
		<form method="POST" action="${mvc.basePath}/projects/${encoder.urlEncode(project.name)}/releases/${empty release.documentId ? '@new' : release.documentId}" enctype="multipart/form-data" data-turbo="false">
			<dl>
				...
				<dd>
					<select name="releaseLicense">
						<optgroup label="${translation.permissive}">
							<option value="Apache License" ${release.licenseType == 'Apache License' or empty release.licenseType ? 'selected="selected"' : ''}><c:out value="${translation.licenseApache}"/></option>
						</optgroup>
						<optgroup label="${translation.copyleft}">
							<option value="GNU Affero GPL3" ${release.licenseType == 'GNU Affero GPL3' ? 'selected="selected"' : ''}><c:out value="${translation.licenseAfferoGPL3}"/></option>
							<option value="GNU GPL3" ${release.licenseType == 'GNU GPL3' ? 'selected="selected"' : ''}><c:out value="${translation.licenseGPL3}"/></option>
							<option value="GNU LGPL3" ${release.licenseType == 'GNU LGPL3' ? 'selected="selected"' : ''}><c:out value="${translation.licenseLGPL3}"/></option>
						</optgroup>
					</select>
				</dd>
				...
				<dd>
					<label>
						<input type="radio" name="releaseReleased" value="true" ${release.released ? 'checked="checked"' : ''} />
						<c:out value="${translation.statusReleased}"/>
					</label>
					<label>
						<input type="radio" name="releaseReleased" value="false" ${not release.released ? 'checked="checked"' : ''} />
